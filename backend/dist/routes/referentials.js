...
// VALIDATION SCHEMAS
// ===================================================================
const regionQuerySchema = joi_1.default.object({
    search: joi_1.default.string().optional(),
    limit: joi_1.default.number().integer().min(1).max(100).default(20),
});
const insuranceQuerySchema = joi_1.default.object({
    type: joi_1.default.string()
        .valid(...Object.values(client_1.InsuranceType))
        .optional(),
...
router.get("/regions", async (req, res) => {
    try {
        ...
        if (search) {
            ...
            filteredRegions = regions.filter((region) => region.name.toLowerCase().includes(searchLower) ||
                region.capital.toLowerCase().includes(searchLower) ||
                region.departments.some((dept) => dept.toLowerCase().includes(searchLower)));
        }
...
// ===================================================================
router.get("/medical-specialties", async (req, res) => {
    try {
        const specialties = [
            {
            ...
            },
            {
                ...
                description: "Paludisme, tuberculose, VIH/SIDA et infections tropicales",
...
router.get("/insurance-companies", async (req, res) => {
    try {
        ...
        if (region) {
            filteredCompanies = companies.filter((company) => company.regionsServed.includes(region));
        }
        res.json({
            success: true,
...
// ===================================================================
router.get("/establishment-types", async (req, res) => {
    try {
        const establishmentTypes = [
            {
                code: "HOSPITAL",
                name: "Hôpital",
                description: "Établissement public de référence avec plateau technique complet",
                characteristics: [
                    "Urgences 24h/24",
                    "Hospitalisation",
                    "Bloc opératoire",
...
//# sourceMappingURL=referentials.js.map